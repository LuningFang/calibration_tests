...
// Output directories
const std::string out_dir = "inputs/ART_calibrate";
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 5){
        std::cout << "useage: ./ART_calibration <0 - Acc, 1 - Turn Left, 2 - Ramp test> <max voltage ratio> <stalling torque> <tire rolling resistance>" <<std::endl;

        return -1;
    }

    std::cout << "chrono output path" << GetChronoOutputPath() << std::endl;


    int test_type = std::stoi(argv[1]);
    double max_voltage_ratio = std::stof(argv[2]);
    double stalling_torque = std::stof(argv[3]);
    double tire_rolling_resistance = std::stof(argv[4]);
    ...
    my_rccar.SetTireRollingResistance(tire_rolling_resistance);
    my_rccar.SetMotorResistanceCoefficients(1,2);
    my_rccar.Initialize();
    ...
    std::cout << "wheel mass: " << my_rccar.GetVehicle().GetWheel(0,RIGHT)->GetMass() << ", tire mass: " << my_rccar.GetVehicle().GetTire(0, RIGHT)->GetMass() << std::endl;
